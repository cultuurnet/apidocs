...
* As another exception, [`childrenOnly`](/docs/uitdatabank/entry-api/reference/operations/update-a-event-children-only) is kept on the event when it is not included in the update request, even though it has a default value. To disable it, explicitly send `"childrenOnly": false`. Sending `"childrenOnly": null` counts as not sending it, so it also keeps the current value.
