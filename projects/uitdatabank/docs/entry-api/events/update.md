...
* The user that your [user access token](https://docs.publiq.be/docs/authentication/methods/user-access-token) belongs to was added as a contributor on the event. *(deprecated system)*
* You have been added as an owner of the organisation that owns the event. For more information on how to manage your organisation and its users, see [this helpdesk article](https://helpdesk.publiq.be/hc/nl/articles/26125476448018-Hoe-kan-ik-mijn-organisatie-beheren).
...
* You can remove one or more optional properties from the event by omitting them from the event data in your update request, or by explicitly setting them to `null` (with the exception of `childrenOnly`, see below). Note that when setting them to `null`, they will be removed completely from the event.
...
* As another exception, `childrenOnly` is kept on the event when it is not included in the update request, even though it has a default value. To disable it, explicitly send `"childrenOnly": false`, or use the [`PUT /events/{eventId}/children-only`](/docs/uitdatabank/entry-api/reference/operations/update-a-event-children-only) endpoint. Sending `"childrenOnly": null` counts as not sending it, so it also keeps the current value.
