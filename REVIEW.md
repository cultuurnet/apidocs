...
- The "same name as the source JSON property" rule above wins over the boolean naming convention in the [API design guidelines](https://docs.publiq.be/docs/api-design-guidelines/rest#naming-conventions-for-properties). A boolean copied straight from the source JSON keeps that exact name, even when it has no `is` or `has` prefix. Examples are `overnight` and `childrenOnly`.
- A boolean that is derived at index time is computed, not copied from the source JSON, so it has no source name to mirror. It gets its own descriptive name with an `is` or `has` prefix. Examples are `hasChildcare`, derived from the `childcare` object, and `isDuplicate`, derived from `duplicateOf`.