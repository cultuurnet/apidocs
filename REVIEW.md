...
- A boolean that is derived at index time is computed, not copied from the source JSON, so it has no source name to mirror. It gets its own descriptive name with an `is` or `has` prefix. For example `isDuplicate` (boolean) is derived from `duplicateOf` (array), and `hasVideos` (boolean) is derived from `videos` (array).