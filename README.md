...
This hash is called the "stable id". You can rename the file or even move it, and links to it will keep working as long as the id stays the same.

However, these stable ids make the URLs harder to read and ugly to share. While it can be overridden, URLs will still follow the `/<stable-id>-<slug>` structure (where the "slug" is generated by Stoplight), so the URLs are not entirely customizable this way.

Instead, the best way to customize your pages URLs is by setting a `slug` property in your `toc.json` file (see [Sidebar](#sidebar-) for more info about `toc.json`).
...
Avoid changing an existing slug later if possible, because the old slug will stop working. If you _need_ to change an existing slug, contact [a Stoplight tool owner](https://confluence.uitdatabank.be/display/PMO/Stoplight.io+-+toolfiche) to add a redirect for your old slug to your new slug. 
